...
Most files are standalone `.cpp` examples. You can compile one directly with a C++ compiler:

```bash
g++ -std=c++20 "Variables /initialization.cpp" -o initialization
./initialization
```

Some examples are intentionally written to explore compiler errors or undefined behavior. Those files may not compile cleanly, or they may produce unexpected output by design.
